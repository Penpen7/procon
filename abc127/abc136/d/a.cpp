...
int main() {
  ...
  vector<pl> storeRL;
  {
    ...
    for (char i : s) {
      if (i == 'R' and isBefR) {
        r++;
      } else if (i == 'R' and !isBefR) {
        storeRL.push_back({r - 1, l - 1});
        r = 1;
        l = 0;
      } else if (i == 'L') {
        l++;
  ...
  rep(i, SIZE(storeRL)) {
    ll x = storeRL[i].F, y = storeRL[i].S;
    ll move_count = max(x, y);
    ll l = (x + y) / 2 + (x + y) % 2 + 1;
    ll r = (x + y) / 2 + 1;
    if (y < x) swap(l, r);
    if (move_count % 2 == 0) swap(l, r);
    rep(j, x) cout << 0 << " ";
    cout << l << " " << r << " ";
    rep(j, y) {
      if (i == SIZE(storeRL) - 1 and j == y - 1) {
        cout << 0 << endl;
      } else {
        cout << 0 << " ";
      }
