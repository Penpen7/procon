...
// 大文字65-90(-32)
// 小文字97-122(+32)
bool check(int i, vector<int> trust, vector<int> lie) {
  for (int k : trust)
...
int main() {
  ...
  int n;
  cin >> n;
  vector<vector<int>> trust(n, vector<int>(0));
  vector<vector<int>> lie(n, vector<int>(0));
  rep(i, n) {
    int a;
    cin >> a;
    rep(j, a) {
  ...
  for (int i = 0; i < (1 << n); i++) {
    ...
    rep(j, n) {
      if (i & (1 << j)) {
        flag &= check(i, trust[j], lie[j]);
      }
    }
    if (flag) temp = __builtin_popcount(i);
    res = max(res, temp);
  }
  cout << res << endl;
  return 0;
