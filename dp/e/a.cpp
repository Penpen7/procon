...
signed main() {
  ...
  ll n, w;
  cin >> n >> w;
  vector<pair<ll, ll>> nw(n);
  // F:weight, S:value
  rep(i, n) cin >> nw[i].F >> nw[i].S;
  // d[i][v]=重さの最小値
  vector<vector<ll>> dp(n, vector<ll>((ll)1e+5 + 1, INF));
  dp[0][0] = 0;
  dp[0][nw[0].S] = nw[0].F;
  repz(kind, 1, n) {
    rep(value, (ll)1e+5 + 1) {
      // not choose
      chmin(dp[kind][value], dp[kind - 1][value]);
      // choose
      int newvalue = value + nw[kind].S;
      if (newvalue < (ll)1e+5 + 1) {
        chmin(dp[kind][newvalue], dp[kind - 1][value] + nw[kind].F);
      }
    }
  }
  for(ll value=(ll)1e+5;value>=0;value--){
    if(dp[n-1][value]<=w){
      cout << value << endl;
      ret;
    }
