...
int main() {
  ...
  cin >> n >> k;
  cin >> r >> s >> p;
  map<char, _score> score;
  score['r'] = {p, 0, 0};
  score['s'] = {0, r, 0};
  score['p'] = {0, 0, s};
  vector<vector<char>> a(k, vector<char>(0));
  rep(i, n) {
    char temp;
    cin >> temp;
    a[i % k].push_back(temp);
  }
  ll res = 0;
  rep(i, k) {
    ll rr = 0, ss = 0, pp = 0;
    ll nrr, nss, npp;
    for (char j : a[i]) {
      nrr = max(ss + score[j].r, pp + score[j].r);
      npp = max(rr + score[j].p, ss + score[j].p);
      nss = max(pp + score[j].s, rr + score[j].s);
      rr = nrr;
      pp = npp;
      ss = nss;
    }
    res += max(rr, max(pp, ss));
  }
  cout << res << endl;
