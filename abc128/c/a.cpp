...
//  ba;
bool check(int bits, int p, vector<int> connect) {
  int sum = 0;
  for (int i : connect) {
    sum += (bool)(bits & (1 << i));
    // debug(i,bits,sum,p);
...
int main() {
  ...
  int n, m;
  cin >> n >> m;
  vector<vector<int>> connect(m, vector<int>(0));
  vector<int> p(m);
  rep(i, m) {
    int k;
    cin >> k;
  ...
  // int light;
  // cin >> light;
  // cout << check(3, p[light], connect[light]) << endl;
  for (int bits = 0; bits < (1 << n); bits++) {
    bool flag = true;
    rep(light, m) { flag &= check(bits, p[light], connect[light]); }
    res += flag;
  }
  cout << res << endl;
  return 0;
