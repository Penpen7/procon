...
signed main() {
  ...
  if (conti > 0) a.push_back(conti);
  ll zeronum = (SIZE(a) + 1) / 2;
  if (zeronum <= k) {
    cout << accumulate(ALL(a), 0LL) << endl;
    return 0;
  }
  vector<ll> b(SIZE(a) + 1);
  b[0] = 0;
  rep(i, SIZE(a)) { b[i + 1] = b[i] + a[i]; }
  ll res = -100;
  rep(i, SIZE(a) + 1) {
    if (i % 2 == 0) {
      if (i + 2 * k > SIZE(a))
        chmax(res, b[SIZE(a)] - b[i]);
      else
        chmax(res, b[i + 2 * k] - b[i]);
    } else {
      if (i + 2 * k + 1 > SIZE(a))
        chmax(res, b[SIZE(a)] - b[i]);
      else
        chmax(res, b[i + 2 * k + 1] - b[i]);
    }
  }
  cout << res << endl;
  return 0;
