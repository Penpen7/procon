...
signed main() {
  ...
  std::cin.tie(nullptr);
  vector<multiset<int>> d(200000, multiset<int>());
  multiset<ll> maxs;
  int n, q;
  cin >> n >> q;
  ...
  rep(i, q) {
    ...
    int rate = a[c].S;
    if (SIZE(d[bef]) > 0)
      maxs.erase(maxs.find((ll) * (d[bef].rbegin())));
    else
      maxs.erase(maxs.find(INF));
    d[bef].erase(d[bef].find(rate));
    if (SIZE(d[bef]) > 0) maxs.insert((ll) * (d[bef].rbegin()));
    else maxs.insert(INF);

    a[c].F = e;
    if (SIZE(d[e]) > 0)
      maxs.erase(maxs.find((ll) * (d[e].rbegin())));
    else
      maxs.erase(maxs.find(INF));
    d[e].insert(rate);
    maxs.insert((ll) * (d[e].rbegin()));
