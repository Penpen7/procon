...
//  ba;
ll binary_search(ll max, ll min, vector<ll> x) {
  return abs(lower_bound(ALL(x), max) - lower_bound(ALL(x), min));
}
int main() {
  // faster
  // C言語流の入出力は使用できない
  ...
  dp[0] = 0;
  repz(i, 0, l) {
    chmin(dp[i + 1], dp[i] + t1 * 1 + t2 * 0 + x[i] * t3);

    if (i + 2 > l)
      chmin(dp[l], dp[i] + t1 / 2 + x[i] * t3 + (2 * l - 2 * i - 1) * t2 / 2);
    else
      chmin(dp[i + 2], dp[i] + 1 * t1 + 1 * t2 + x[i] * t3);

    if (i + 4 > l)
      chmin(dp[l], dp[i] + t1 / 2 + x[i] * t3 + (2 * l - 2 * i - 1) * t2 / 2);
    else
      chmin(dp[i + 4], dp[i] + 1 * t1 + 3 * t2 + x[i] * t3);
  }
  cout << dp[l] << endl;
  return 0;
}
