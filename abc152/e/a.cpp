...
signed main() {
  ...
  repz(i, 1, n) {
    ll g = _gcd(a[i], a[0]);
    preb[i] = MP(a[0] / g, a[i] / g);
    denominator.insert(a[i] / g);
  }
  map<ll, ll> l;
  for (ll i : denominator) {
    if (SIZE(l) == 0) {
      l = decompositPrime(i);
      continue;
    }
    map<ll, ll> temp = decompositPrime(i);
    for (pair<ll, ll> j : temp) {
      ll newpower = max(j.second, l[j.first]);
      l[j.first] = newpower;
    }
  }
  ...
  for (pair<ll, ll> i : l) {
    ...
    rep(j, i.second) { b[0] *= i.first; }
  }
  vector<pair<mint, mint>> prebm(n);
  rep(i, n) {
    prebm[i].F = preb[i].F;
    prebm[i].S = preb[i].S;
  }
  repz(i, 1, n) { b[i] = prebm[i].first / prebm[i].second * b[0]; }
  mint res = 0;
  rep(i, n) { res += b[i]; }
  cout << res << endl;
  return 0;
